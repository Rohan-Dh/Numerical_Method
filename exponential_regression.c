#include <stdio.h>
#include <math.h>
#define f(x, a, b) a* pow(exp(1), b*x)
...
void find_regression(float x[], float y[], int total_data, float point)
{
    ...
    slope = sum1 / sum2;
    b = pow(exp(1), (y_mean-(slope*x_mean)));

    float value = f(point, b, slope);
...
int main()
{
    ...
    {
    case '1':
        find_regression(lgtm, X, n, x);
        break;
    case '2':
        find_regression(X, lgtm, n, x);
        break;
    case '3':
        return (0);
    default:
